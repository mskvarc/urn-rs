...
fn bench_builder(c: &mut Criterion) {
    ...
    g.bench_function("build_minimal", |b| {
        b.iter(|| {
            black_box(
                UrnBuilder::new(black_box("example"), black_box("1234:5678"))
                    .build()
                    .unwrap(),
            )
        })
...
fn bench_accessors(c: &mut Criterion) {
    ...
    g.bench_function("r_component", |b| {
        b.iter(|| black_box(black_box(&urn).r_component()))
    });
    g.bench_function("q_component", |b| {
        b.iter(|| black_box(black_box(&urn).q_component()))
    });
    g.bench_function("f_component", |b| {
        b.iter(|| black_box(black_box(&urn).f_component()))
    });
    g.finish();
    ...
    let urn_a = urn.clone();
    let urn_b = urn.clone();
    g.bench_function("eq_same_all_components", |b| {
        b.iter(|| black_box(black_box(&urn_a) == black_box(&urn_b)))
    });
